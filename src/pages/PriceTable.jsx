...
import { 
  ...
  Scale, UploadCloud, CheckSquare, Trash2, ChevronUp, ChevronDown, ArrowUpDown, Layers, Tag
} from 'lucide-react';
...
export default function PriceTable() {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#f8fafc', fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
        ...
        {/* HEADER ESTÁTICO E IMÓVEL */}
        <div style={{ flexShrink: 0, zIndex: 50, position: 'relative' }}>
            <Header title="Tabela de Preços" />
        </div>

        {/* CONTAINER DINÂMICO DE ROLAGEM */}
        <div className="custom-scrollbar" style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', display: 'flex', flexDirection: 'column' }}>
            
            {/* WRAPPER INTERNO PARA LIMITAR LARGURA E DAR ESPAÇAMENTO */}
            <div style={{ minHeight: '100%', display: 'flex', flexDirection: 'column', maxWidth: '1600px', width: '100%', margin: '0 auto', padding: '1rem 1.5rem', gap: '0.75rem' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                            ...
                            <div style={{ flex: '1 1 140px' }}>
                                ...
                                <select value={paymentTerm} onChange={e => setPaymentTerm(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: '8px', border: 'none', outline: 'none', background: '#ffffff', color: '#0f172a', fontSize: '0.85rem', fontWeight: 600, boxShadow: '0 2px 5px rgba(0,0,0,0.1)', cursor: 'pointer' }}>
                                    <option value="0.1360">0 Dias</option><option value="0.1287">15 Dias</option><option value="0.1262">20 Dias</option><option value="0.1213">30 Dias</option><option value="0.1103">55 Dias</option><option value="0.1066">60 Dias</option><option value="0.0919">90 Dias</option><option value="0.0772">120 Dias</option><option value="0.0919">30/300 Dias</option>
                                </select>
                            </div>
                            <div style={{ flex: '1 1 140px' }}>
                                ...
                                <select value={clientTier} onChange={e => setClientTier(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: '8px', border: 'none', outline: 'none', background: '#ffffff', color: '#0f172a', fontSize: '0.85rem', fontWeight: 600, boxShadow: '0 2px 5px rgba(0,0,0,0.1)', cursor: 'pointer' }}>
                                    <option value="0">Padrão</option><option value="0.09">Ouro (9%)</option><option value="0.12">Diamante (12%)</option><option value="0.09">E-commerce</option>
                                </select>
                            </div>
                        </div>
                    )}
                ...
                <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                    ...
                    {/* BOTÃO DROPDOWN DE SETOR DE ATIVIDADE */}
                ...
                {/* TABELA - COM minHeight FIXO PARA EVITAR O ESMAGAMENTO NA TELA MENOR */}
                <div style={{ flex: 1, minHeight: '400px', display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.02)' }}>
                        ...
                        <table style={{ width: '100%', minWidth: '1000px', borderCollapse: 'collapse' }}>
                            ...
                            <tbody>
                                ...
                                currentProducts.map((product) => {
                                    ...
                                    // INTELIGÊNCIA DA MINIATURA: Busca a foto com o nome mais curto
                                    let displayImgUrl = product.imageUrl; 
                                    if (product.images && product.images.length > 0) {
                                        const fbImgs = product.images.filter(img => img.type === 'fundo_branco');
                                        if (fbImgs.length > 0) {
                                            fbImgs.sort((a, b) => (a.name?.length || 0) - (b.name?.length || 0));
                                            displayImgUrl = fbImgs[0].url;
                                        }
                                    }
                                        ...
                                        {visibleCols.img && (
                                            <td style={{ padding: '6px', textAlign: 'center' }}>
                                                <div style={{ position: 'relative', width: 32, height: 32, margin: '0 auto', background: '#fff', borderRadius: '4px', border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                    {displayImgUrl ? ( <img src={displayImgUrl} alt="" loading="lazy" decoding="async" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain', borderRadius: '4px' }} onError={(e) => { e.target.style.display = 'none'; e.target.nextElementSibling.style.display = 'block'; }} /> ) : null}
                                                    <ImageIcon size={14} color="#cbd5e1" style={{ display: displayImgUrl ? 'none' : 'block' }} />
                                                </div>
                                            </td>
                                        )}
                                        {visibleCols.sku && (
                ...
                <div style={{ flex: 1, display: 'flex', padding: '20px 30px', overflowX: 'auto', overflowY: 'hidden', alignItems: 'flex-start' }}>
                    <div style={{ display: 'flex', gap: '16px', minWidth: 'max-content' }}>
                        ...
                        {compareList.map(prod => {
                            let compImgUrl = prod.imageUrl; 
                            if (prod.images && prod.images.length > 0) {
                                const fbImgs = prod.images.filter(img => img.type === 'fundo_branco');
                                if (fbImgs.length > 0) {
                                    fbImgs.sort((a, b) => (a.name?.length || 0) - (b.name?.length || 0));
                                    compImgUrl = fbImgs[0].url;
                                }
                            }
                            return (
                            <div key={prod.id} style={{ width: '280px', flexShrink: 0, background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '15px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                                ...
                                <div style={{ height: ROW_HEIGHTS.header, display: 'flex', flexDirection: 'column' }}>
                                    <div style={{ height: '70px', display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: '10px', background: '#fff' }}>
                                        {compImgUrl ? <img src={compImgUrl} loading="lazy" decoding="async" style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} /> : <ImageIcon size={30} color="#cbd5e1" />}
                                    </div>
            ...
            <>
                ...
                <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '100%', maxWidth: '400px', background: '#fff', zIndex: 9999, overflowY: 'auto', boxShadow: '-10px 0 30px rgba(0,0,0,0.1)', animation: 'slideInRight 0.2s ease-out', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ background: '#fff', padding: '20px', display: 'flex', justifyContent: 'center', position: 'relative', borderBottom: '1px solid #e2e8f0', minHeight: '220px', alignItems: 'center', flexShrink: 0 }}>
                        ...
                        {(() => {
                            let drawerImgUrl = selectedProduct.imageUrl; 
                            if (selectedProduct.images && selectedProduct.images.length > 0) {
                                const fbImgs = selectedProduct.images.filter(img => img.type === 'fundo_branco');
                                if (fbImgs.length > 0) {
                                    fbImgs.sort((a, b) => (a.name?.length || 0) - (b.name?.length || 0));
                                    drawerImgUrl = fbImgs[0].url;
                                }
                            }
                            return drawerImgUrl ? (
                                <img src={drawerImgUrl} alt="" loading="lazy" decoding="async" style={{ maxWidth: '90%', maxHeight: '180px', objectFit: 'contain' }} onError={(e) => { e.target.style.display = 'none'; }} />
                            ) : (
