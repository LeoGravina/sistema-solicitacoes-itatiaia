...
export default function ProductAnalysis() {
    ...
    return (
        // SOLUÇÃO DO SCROLL: Apenas div limpa sem travar altura!
            ...
            <div style={{maxWidth:'1300px', margin:'0 auto', padding:'2rem'}}>
                ...
                {!product && !loading && (
                    <div style={{textAlign:'center', marginTop:'60px', color:'#94a3b8', display:'flex', flexDirection:'column', alignItems:'center'}}>
                        <div style={{background:'#f8fafc', padding:'30px', borderRadius:'50%', marginBottom:'20px'}}><Package size={60} style={{opacity:0.3}}/></div>
                    ...
                    <div style={{display:'flex', gap:'5rem', alignItems:'flex-start', flexWrap: 'wrap', animation: 'fadeIn 0.4s ease-out'}}>
                        <div style={{flex: '1 1 500px', display:'flex', flexDirection:'column'}}>
                            <div style={{display:'flex', justifyContent:'center', alignItems:'center', background:'#f8fafc', borderRadius:'24px', padding:'40px', minHeight:'500px', border:'1px solid #f1f5f9'}}>
                                {currentMainImg ? <img src={currentMainImg} alt={product.description} style={{maxWidth:'100%', maxHeight:'600px', objectFit:'contain', mixBlendMode:'multiply'}} onError={(e) => { e.target.style.display = 'none'; e.target.nextElementSibling.style.display = 'flex'; }} /> : null}
                                <div style={{display: currentMainImg ? 'none' : 'flex', flexDirection:'column', alignItems:'center', color:'#cbd5e1'}}><ImageIcon size={60} strokeWidth={1} /><p style={{fontSize:'1rem', marginTop:16}}>Imagem não encontrada</p></div>
                            </div>
                        ...
                        <div style={{flex: '1 1 450px'}}>
                            ...
                            <h1 style={{margin:'0 0 40px 0', fontSize:'2.5rem', color:'#0f172a', lineHeight:'1.2', fontWeight:800, letterSpacing:'-1px'}}>{product.description}</h1>

                            <div style={{borderTop:'1px solid #e2e8f0', paddingTop:'40px', paddingBottom:'60px'}}>
                                ...
                                {product.dimensions ? (
                                    <div style={{display:'flex', flexDirection:'column', gap:'16px'}}>
                                        <div style={{display:'flex', justifyContent:'space-between', paddingBottom:'16px', borderBottom:'1px solid #f1f5f9'}}><span style={{color:'#64748b', fontWeight:500}}>Dimensões (C x L x A)</span><strong style={{color:'#0f172a'}}>{product.dimensions.length} x {product.dimensions.width} x {product.dimensions.height} mm</strong></div>
                                        <div style={{display:'flex', justifyContent:'space-between', paddingBottom:'16px', borderBottom:'1px solid #f1f5f9'}}><span style={{color:'#64748b', fontWeight:500}}>Peso Bruto / Líquido</span><strong style={{color:'#0f172a'}}>{product.dimensions.weightBruto} kg / {product.dimensions.weightLiq} kg</strong></div>
                                        <div style={{display:'flex', justifyContent:'space-between', paddingBottom:'16px', borderBottom:'1px solid #f1f5f9'}}><span style={{color:'#64748b', fontWeight:500}}>Volume Ocupado</span><strong style={{color:'#0f172a'}}>{product.dimensions.volume} m³</strong></div>
                                        <div style={{display:'flex', justifyContent:'space-between', paddingBottom:'16px', borderBottom:'1px solid #f1f5f9'}}><span style={{color:'#64748b', fontWeight:500}}>Fator Cubagem (KG³)</span><strong style={{color:'#0f172a'}}>{product.dimensions.kg3} kg</strong></div>
                                        <div style={{display:'flex', justifyContent:'space-between', paddingBottom:'16px', borderBottom:'1px solid #f1f5f9'}}><span style={{color:'#64748b', fontWeight:500}}>Classificação Comercial</span><strong style={{color:'#0f172a'}}>{product.dimensions.classificacao}</strong></div>
                                        <div style={{display:'flex', justifyContent:'space-between', paddingBottom:'16px', borderBottom:'1px solid #f1f5f9'}}><span style={{color:'#64748b', fontWeight:500}}>Status Ativo (Linha / SKU)</span><strong style={{color:'#10b981'}}>{product.dimensions.statusLinha} / {product.dimensions.statusSku}</strong></div>
                                    </div>
